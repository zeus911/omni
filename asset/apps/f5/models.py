...
class HostNodeTemplateModel(models.Model):
    """
    主机节点配置模板
    """
    name = models.CharField(max_length=30, help_text='主机节点配置模板名称.')
    workspace_path = models.FilePathField(blank=True, null=True, max_length=500, help_text='默认工作目录')
    run_user = models.IntegerField(blank=True, null=True, help_text='该环境中,通用的运行服务或执行指令的用户,对应user表ID')
    software_list = list_fields.ListCommaSeparatedIntegerField(blank=True, null=True, max_length=512,
                                                               help_text='该节点中运行的软件或服务')
    ...
    class Meta:
        db_table = 'host_node_template'


class HostNodeModel(models.Model):
    """
    主机节点
    """
    sn = models.CharField(max_length=20, unique=True, help_text='主机节点SN编号,用于唯一标识服务器,不可变')
    hostname = models.CharField(max_length=20, unique=True, help_text='主机节点主机名, 可以被修改')
    visible_name = models.CharField(blank=True, null=True, max_length=30, help_text='显示名称')
    host_identity = models.CharField(max_length=30, unique=True, help_text='服务器标识,用于日常运维操作中的标识,'
                                                                           '需与saltstack等运维平台中的服务器ID兼容,由程序自动生成,'
                                                                           '如：shanghai-ytoxl-P01-SAP-01')
    description = models.CharField(blank=True, null=True, max_length=1024, help_text='详细描述')
    workspace_path = models.FilePathField(blank=True, null=True, max_length=500, help_text='默认工作目录')
    template_list = list_fields.ListCommaSeparatedIntegerField(blank=True, null=True, max_length=30,
                                                               help_text='主机配置模板, 对应template表id')
    run_user = models.IntegerField(blank=True, null=True, help_text='默认的运行服务或执行指令的用户,对应user表ID')
    software_list = list_fields.ListCommaSeparatedIntegerField(blank=True, null=True, max_length=512,
                                                               help_text='该节点中运行的软件或服务')
    model = models.SmallIntegerField(blank=True, null=True, help_text='服务器型号,为server_model表ID')
